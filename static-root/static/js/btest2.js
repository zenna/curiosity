...
bTest.prototype.setBodies = function(bodyEntities) {
    ...
    for(var id in bodyEntities) {
        ...
        if(entity.id == 'ground' || entity.dynamic === false) {
            bodyDef.type = b2Body.b2_staticBody;
        }
        else {
...
bTest.prototype.addDistanceJoint = function(body1Id, body2Id, params) {
    ...
    if(params && params['frequencyHz'])
        joint.frequencyHz = params['frequencyHz'];
    if(params && params['dampingRatio'])
        joint.dampingRatio = params['dampingRatio'];
    this.world.CreateJoint(joint);
}
...
bTest.prototype.applyForce = function(bodyId, degrees, power) {
	var body = this.bodiesMap[bodyId];
	// Convert degrees into radians and li
	body.ApplyForce(new b2Vec2(Math.cos(degrees * (Math.PI / 180)) * power, Math.sin(degrees * (Math.PI / 180)) * power), body.GetWorldCenter());
}
